...
{   
    ...
    public ArrayList<String> resXml = new ArrayList<String>();
    public HashMap<String, String> resMapXml = new HashMap<String,String>();
    public String annio = "";
...
//------------------------------------------------------------SERVLET.INIT------------------------------------------------------------------------        
    public void init(ServletConfig config) throws ServletException
    {
        ...
        File a= new File(context.getRealPath("p2/examen.xsd"));
        String dir = f.getAbsolutePath();
        String dira = a.getAbsolutePath();        
	...
	//Creada batería de parsers
        ...
        dbf1.setAttribute(JAXP_SCHEMA_LANGUAGE, W3C_XML_SCHEMA);
        dbf1.setAttribute(JAXP_SCHEMA_SOURCE, dira);
        ...
        try
        {
            ...
            ErrorHandler er1 = new ErrorHandler(); //Creacion del objeto error
            //Asingación de la clase de control de errores al parser 
            db1.setErrorHandler(er1);
        }catch(ParserConfigurationException pce)
        {
            pce.printStackTrace();
        }
	    ...
	    else
	    {
            ...
            else
            {
                ...
                {                        
                    ...
                    case "15": doGetFase15(res,auto); break;
                }
            }
	    }        
    }//doGet
...
//---------------------------------------------------------FUNCTIONS------------------------------------------------------------------------
    ...
    public void getContentXml()
    {
        String domExam = "examen";
        //String comentario = "";
        String color = "";
        String pais = "";
        int i = 0;        
        Document res = null;
        Iterator it = mapDocs.keySet().iterator();
        Boolean isFirst = false;
        while(it.hasNext())
        {
            String key = (String)it.next();
            if(key.equals(domExam))
            {
                res = mapDocs.get(key);
            }
        }
        ...
        for (i = 0;i<nodos.getLength();i++)
        {
            Node first = nodos.item(i);
            if(first.getNodeName().equals("pais"))
            {
                pais = first.getTextContent();
                //resXml.add(cancion);
                color = first.getAttributes().getNamedItem("decoracion").getTextContent();
                resMapXml.put(pais,color);
            }
            else if(first.getNodeType() == org.w3c.dom.Node.TEXT_NODE)
            {                                              
                if(!isFirst)
                {
                    annio = first.getNodeValue();
                    isFirst = true;
                }
                else
                {
                    annio = annio.concat(first.getNodeValue());
                }                                
            }
        }
    ...
    {                            
        ...
        out.println("<link rel='stylesheet' type='text/css' href='p2/iml.css'></link>"); //href='iml.css' en el lab
        out.println("</head>");
        out.println("<body>");
        ...
        out.println("<a href = '/testP2P3/P2IM?p=d4r18c392b&pfase=02'>Pulsa aqu&iacute; para ver los ficheros err&oacute;neos</a>");
        ...
        out.println("<br></br>");
        out.print("<p>"+annio+"</p>");
        Iterator it = resMapXml.keySet().iterator();
        while(it.hasNext())
        {
            String key = (String)it.next();
            out.print("<p> Pais: "+key+" Color :"+resMapXml.get(key)+"</p>");
        }
        /*for(int i=0;i<resXml.size();i++)
        {            
            out.print("<p>"+resXml.get(i)+"</p>");
        }*/
        out.println("</body>");
        out.println("<footer>");
    ...
    {	               
        ...
        out.println("<meta charset='utf-8'></meta>");
        out.println("<title>Sint: Pr&aacute;ctica 2. Consulta de canciones</title>");
	    out.println("<link rel='stylesheet' type='text/css' href='p2/iml.css'></link>");
        out.println("</head>");
        out.println("<body>");
        out.println("<h1>Servicio de consulta de canciones</h1>");	    
        ...
        out.println("<button class = 'buttonAtras'onclick=\"window.location='/testP2P3/P2IM?p=d4r18c392b&pfase=01'\">Atr&aacute;s</button>");
        out.println("</body>");
        out.println("<footer>");
        out.println("<p>sint48. @Diego R&iacute;os Castro.</p>");                
        out.println("</footer>");
    ...
    public void doGetFase15(HttpServletResponse res, String auto)throws IOException
    {        
        doHtmlF15(res);         
    }
    ...
    {                              
        ...
        out.println("<input type = 'submit' class = 'buttonSubmit'></input>");            
        out.println("</form>");
        out.println("<button type = 'button' class = 'buttonAtras' onclick=\"window.location='/testP2P3/P2IM?p=d4r18c392b&pfase=01'\">Atr&aacute;s</button> ");
        out.println("</body>");
        out.println("<footer>");
        out.println("<p>sint48. @Diego R&iacute;os Castro.</p>");                
        out.println("</footer>");
    ...
    {                              
        ...
        out.println("<link rel='stylesheet' type='text/css' href='p2/iml.css'></link>");
        /*out.println("<style>");
        out.println(".primero{color: red; font-style: italic}");
        out.println("</style>");*/
        out.println("</head>");
        out.println("<body>");
        out.println("<h1>Servicio de consulta de canciones</h1>");    
        ...
        for(int i=0;i<listaDiscos.size();i++)
        {
            ...
            /*if(d.getTitulo(d).equals(resXml.get(0)))
            {
                if(i==0)
                {
                    out.println("<p class='esDisco'><input type = 'radio' name = 'pidd' value = "+d.getIDD(d)+" checked>"+Integer.toString(i+1)+".-"+" T&iacute;tulo ='"+d.getTitulo(d)+"' --- IDD ='"+d.getIDD(d)+"' --- Int&eacute;rprete ='"+d.getInterprete(d)+"' --- Idiomas ='"+d.getIdiomas(d)+"'</input></p>");                    
                }                
                else 
                {
                    out.println("<p class='esDisco'><input type = 'radio' name = 'pidd' value = "+d.getIDD(d)+">"+Integer.toString(i+1)+".-"+" T&iacute;tulo ='"+d.getTitulo(d)+"' --- IDD ='"+d.getIDD(d)+"' --- Int&eacute;rprete ='"+d.getInterprete(d)+"' --- Idiomas ='"+d.getIdiomas(d)+"'</input></p>");       
            ...
            else
            {
                ...
                {
                    out.println("<p><input type = 'radio' name = 'pidd' value = "+d.getIDD(d)+">"+Integer.toString(i+1)+".-"+" T&iacute;tulo ='"+d.getTitulo(d)+"' --- IDD ='"+d.getIDD(d)+"' --- Int&eacute;rprete ='"+d.getInterprete(d)+"' --- Idiomas ='"+d.getIdiomas(d)+"'</input></p>");       
        ...
        out.println("<input type = 'submit' class = 'buttonSubmit'></input>");
        out.println("</form>");
        out.println("<button class = 'buttonAtras' onclick=\"window.location='/testP2P3/P2IM?p=d4r18c392b&pfase=11'\">Atr&aacute;s</button> ");
        out.println("<br></br>");
        out.println("<button class = 'buttonInicio' onclick=\"window.location='/testP2P3/P2IM?p=d4r18c392b&pfase=01'\">Inicio</button> ");
        out.println("</body>");
        out.println("<footer>");
        out.println("<p>sint48. @Diego R&iacute;os Castro.</p>");                
        out.println("</footer>");
    ...
    {                              
        ...
        {  
            ...
            /*String dur = c.getDuracion(c);
            modulo = Integer.parseInt(dur) % 2;
            if(modulo == 0)
            {
                if(resXml.get(1).equals("par"))
                {
                    if(i==0)
                    {
            ...
            /*if(Integer.parseInt(resXml.get(0)) > Integer.parseInt(dur))
            {
                me = true;
                if(resXml.get(1).equals("menor"))
                {
                    if(i==0)
                    {
                        out.println("<p><input type = 'radio' name = 'pidc' value = "+c.getIdc(c)+" checked>"+Integer.toString(i+1)+".-"+" T&iacute;tulo ='"+c.getTitulo(c)+"' --- IDC ='"+c.getIdc(c)+"' --- G&eacutenero ='"+c.getGenero(c)+"' --- Duraci&oacute;n ='"+c.getDuracion(c)+" seg.'</input></p>");
        ...
        out.println("<input type = 'submit' class = 'buttonSubmit'></input>");
        out.println("</form>");
        out.println("<button class = 'buttonAtras' onclick=\"window.location='/testP2P3/P2IM?p=d4r18c392b&pfase=12&panio="+anio+"'\">Atr&aacute;s</button> ");
        out.println("<br></br>");    
        out.println("<button class = 'buttonInicio' onclick=\"window.location='/testP2P3/P2IM?p=d4r18c392b&pfase=01'\">Inicio</button> ");
        out.println("</body>");
        out.println("<footer>");
    ...
    {                              
        ...
        out.println("<input type = 'hidden' name = 'p' value = 'd4r18c392b'></input>");
        out.println("<input type = 'hidden' name = 'pfase' value = '15'></input>");	
        out.println("<input type = 'submit' class = 'buttonSubmit'></input>");        
        out.println("</form>");
        out.println("<button class = 'buttonAtras'  onclick=\"window.location='/testP2P3/P2IM?p=d4r18c392b&pfase=13&panio="+anio+"&pidd="+idd+"'\">Atr&aacute;s</button> ");
        out.println("<br></br>");
        out.println("<button class = 'buttonInicio' onclick=\"window.location='/testP2P3/P2IM?p=d4r18c392b&pfase=01'\">Inicio</button> ");
        out.println("</body>");
        out.println("<footer>");
    ...
    public void doHtmlF15(HttpServletResponse res)throws IOException
    {
        PrintWriter out = res.getWriter();        
        out.println("<html>");
        out.println("<head>");
        out.println("<title>Sint: Pr&aacute;ctica 2. Consulta de canciones</title>");
        out.println("<meta charset='utf-8'></meta>");
        out.println("<link rel='stylesheet' type='text/css' href='p2/iml1.css'></link>");
        out.println("</head>");
        out.println("<body>");
        out.println("<h1>Servicio de consulta de canciones</h1>");
        out.println("<form name = 'miformfase15'>");
        //out.println("<h2>Consulta 1: A&ntilde;o="+anio+", Disco="+idd+", Canci&oacute;n="+idc+"</h2>");    
        out.println("<h3>Este es el resultado de la fase 15:</h3>");
        out.println("<p>A&ntilde;o: "+annio+"</p>");          
        Iterator it = resMapXml.keySet().iterator();
        while(it.hasNext())
        {
            String key = (String)it.next();
            out.print("<p> Pais: "+key+"</p>");
        }
